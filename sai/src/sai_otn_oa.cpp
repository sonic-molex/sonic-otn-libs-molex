    ...
    *otn_oa_id = obj->sai_object_id;
    ...
    if (ret_status == SAI_STATUS_SUCCESS)
    {
        ...
        for (uint32_t i = 0; i < attr_count; i++) {
            tmp_status = set_otn_oa_attribute(obj->sai_object_id, attr_list + i);
        ...
        // Program thresholds only when every attribute (including NAME) was set
        // successfully, and only when this device actually has a threshold config.
        if (tmp_status == SAI_STATUS_SUCCESS) {
